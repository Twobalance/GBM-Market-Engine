...
> *Application:* See pages 127–129 for the derivation of the Jump-Diffusion SDE and the Poisson arrival mechanism implemented in the `MarketEngine` class.

**[Glasserman, 2003]** Glasserman, P. (2003). *Monte Carlo Methods in Financial Engineering*. Springer Science & Business Media.  
> *Application:* See Section 3.2 (pp. 93–101) for Geometric Brownian Motion simulation and Section 3.5 (pp. 134–140) for the numerical discretization of jump processes used in the `tick()` function.
...
> *Application:* Refer to page 224 for the "Stylized Facts" (Points 2, 6, and 10) that justify the implementation of Heavy Tails (via Merton Jumps), Volatility Clustering (via Dynamic Volatility), and the Correlation between Volume and Price Volatility in this framework.
